...
from deepglyco.deeplib.gpep.ms2.model import GlycoPeptideMS2TreeLSTM
...
def load_gpep_ms2_dataset(ctx: Context, speclib_files: list[str]):
    datasets: list[GlycoPeptideMS2Dataset] = []
...
def load_pretrained_model(trainer: GlycoPeptideMS2Trainer, model_file: str):
    trainer.build_model()
    ...
    if model_type == trainer.get_config("model", "type", typed=str):
        ...
        trainer.model, GlycoPeptideMS2TreeLSTM
    ):
...
def load_pretrained_model_pepms2(
    model: GlycoPeptideMS2TreeLSTM, state_dict: Mapping[str, Any]
):
    states = {}
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Train a model for glycopeptide MS2.")
    ...
    if train_dir is None:
        train_dir = (
            f"deepgpms2_{time.strftime('%Y%m%d%H%M%S')}"
