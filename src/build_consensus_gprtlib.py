...
def calibrate_rtlib(source_file, reference_file, logger=None):
    ...
    if logger:
        ...
        logger.info(
            f"Source: {speclib.num_retention_time} entries of {speclib.num_glycopeptides} glycopeptides"
        )

    data = speclib.get_retention_time_data()
    assert data is not None
    data = data.drop(columns=[c for c in data.columns if c.endswith("_id")])
...
def plot_rt_calibration(
    ...
    with PdfPages(pdf_path) as pdf:
        if COLUMN_RUN_NAME in rt_data.columns:
...
def build_consesus_rtlib(source_file, reference_file, dest_file):
    ...
    logger.info(
        f"Total: {nr_speclib.num_retention_time} entries of {nr_speclib.num_glycopeptides} glycopeptides"
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Build consensus glycopeptide retention time library."
    )
    parser.add_argument(
